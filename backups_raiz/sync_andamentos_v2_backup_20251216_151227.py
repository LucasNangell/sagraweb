...
class AndamentosSynchronizer:
    ...
    def mark_as_deleted(self, codstatus: str, nro: int, ano: int, origem: str, motivo: str = None):
        """Registra um CodStatus como excluído definitivamente"""
        try:
            conn = self.conn_mgr.get_mysql()
            cursor = conn.cursor()
    ...
    def detect_and_register_deletions(self, mysql_codes: set, mdb_all_codes: set):
        ...
        try:
            ...
            for codstatus in excluidos_no_mdb:
                ...
                # Marcar como excluído definitivamente
    ...
    def insert_mysql(self, andamento: Dict, origem: str):
        ...
        try:
            ...
            # ===================================================================
            # VERIFICAÇÃO CRÍTICA: Bloquear reinserção de registros excluídos
            # ===================================================================
            if self.is_deleted(codstatus):
                self.logger.logger.warning(
                    f"[BLOQUEADO] Tentativa de inserir CodStatus {codstatus} que está "
                    f"marcado como excluído. Inserção bloqueada."
                )
                self.logger.log(
                    'INSERT_BLOCKED', origem, 'MySQL', codstatus,
                    andamento.get('NroProtocoloLink'), andamento.get('AnoProtocoloLink'),
                    'Bloqueado: registro está na lista de exclusões'
                )
                return  # NÃO INSERIR
    ...
    def insert_mdb(self, andamento: Dict, conn: pyodbc.Connection, destino: str):
        ...
        try:
            ...
            if self.is_deleted(codstatus):
                self.logger.logger.warning(
                    f"[BLOQUEADO] Tentativa de inserir CodStatus {codstatus} no MDB que está "
                    f"marcado como excluído. Inserção bloqueada."
                )
                self.logger.log(
